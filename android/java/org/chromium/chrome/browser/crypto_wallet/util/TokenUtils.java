...
public class TokenUtils {
    ...
    /* Filter tokens by type and add native token.
      ...
      *   - If exists, replace the BlockchainToken object with the one generated by Utils.makeNetworkAsset
      *   - If not exist, insert the BlockchainToken object generated by Utils.makeNetworkAsset to front
      *
    ...
    public static void getUserAssetsFiltered(BraveWalletService braveWalletService,
        ...
        braveWalletService.getUserAssets(selectedNetwork.chainId, coinType, (BlockchainToken[] tokens) -> {
    ...
    public static void getUserOrAllTokensFiltered(
            ...
            NetworkInfo selectedNetwork, int coinType, TokenType tokenType,
            boolean userAssetsOnly, Callbacks.Callback1<BlockchainToken[]> callback) {
        if (userAssetsOnly)
            getUserAssetsFiltered(braveWalletService, selectedNetwork, coinType,
                    tokenType, userAssets -> {
    ...
    public static void getBuyTokensFiltered(BlockchainRegistry blockchainRegistry,
            ...
            Callbacks.Callback1<BlockchainToken[]> callback) {
        blockchainRegistry.getBuyTokens(OnRampProvider.WYRE, selectedNetwork.chainId,
                tokens -> {
                    BlockchainToken[] filteredTokens = filterTokens(selectedNetwork,
    ...
    public static boolean isSameToken(BlockchainToken token1, BlockchainToken token2) {
        if (token1.chainId.equals(token2.chainId) && token1.symbol.equals(token2.symbol)
                ...
                && token1.contractAddress.toLowerCase(Locale.getDefault()).equals(
                        token2.contractAddress.toLowerCase(Locale.getDefault()))) {
            return true;
        }
    ...
    public static void getExactUserAsset(BraveWalletService braveWalletService, NetworkInfo selectedNetwork, int coinType,
        ...
        getUserAssetsFiltered(
                braveWalletService, selectedNetwork, coinType, TokenUtils.TokenType.ALL, userAssets -> {
