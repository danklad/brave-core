...
namespace {
const GraphMLAttr* const attr_name_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "attr name");
const GraphMLAttr* const attr_value =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "attr value");
const GraphMLAttr* const before_node_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "before", kGraphMLAttrTypeLong);
const GraphMLAttr* const binding_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "binding");
const GraphMLAttr* const binding_event_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "binding event");
const GraphMLAttr* const binding_type_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "binding type");
const GraphMLAttr* const block_type_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "block type");
const GraphMLAttr* const call_args_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "args");
const GraphMLAttr* const edge_type_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "edge type");
const GraphMLAttr* const event_listener_id_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge,
                    "event listener id",
                    kGraphMLAttrTypeLong);
const GraphMLAttr* const frame_id_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "frame id");
const GraphMLAttr* const host_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "host");
const GraphMLAttr* const incognito_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "incognito");
const GraphMLAttr* const is_deleted_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode,
                    "is deleted",
                    kGraphMLAttrTypeBoolean);
const GraphMLAttr* const is_style_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge,
                    "is style",
                    kGraphMLAttrTypeBoolean);
const GraphMLAttr* const key_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "key");
const GraphMLAttr* const method_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "method");
const GraphMLAttr* const node_id_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "node id", kGraphMLAttrTypeLong);
const GraphMLAttr* const node_text_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "text");
const GraphMLAttr* const node_type_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "node type");
const GraphMLAttr* const page_graph_edge_id_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "id", kGraphMLAttrTypeLong);
const GraphMLAttr* const page_graph_node_id_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "id", kGraphMLAttrTypeLong);
const GraphMLAttr* const page_graph_edge_time_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "timestamp", kGraphMLAttrTypeLong);
const GraphMLAttr* const page_graph_node_time_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "timestamp", kGraphMLAttrTypeLong);
const GraphMLAttr* const parent_node_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "parent", kGraphMLAttrTypeLong);
const GraphMLAttr* const primary_pattern_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "primary pattern");
const GraphMLAttr* const request_id_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge,
                    "request id",
                    kGraphMLAttrTypeLong);
const GraphMLAttr* const request_type_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "request type");
const GraphMLAttr* const resource_type_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "resource type");
const GraphMLAttr* const response_hash_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "response hash");
const GraphMLAttr* const rule_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "rule");
const GraphMLAttr* const script_id_for_edge_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "script id", kGraphMLAttrTypeLong);
const GraphMLAttr* const script_id_for_node_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "script id", kGraphMLAttrTypeLong);
const GraphMLAttr* const script_position_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge,
                    "script position",
                    kGraphMLAttrTypeLong);
const GraphMLAttr* const script_type_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "script type");
const GraphMLAttr* const secondary_pattern_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "secondary pattern");
const GraphMLAttr* const source_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "source");
const GraphMLAttr* const status_type_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "status");
const GraphMLAttr* const success_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode,
                    "is success",
                    kGraphMLAttrTypeBoolean);
const GraphMLAttr* const tag_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "tag name");
const GraphMLAttr* const url_attr =
    new GraphMLAttr(kGraphMLAttrForTypeNode, "url");
const GraphMLAttr* const value_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "value");
const GraphMLAttr* const size_attr =
    new GraphMLAttr(kGraphMLAttrForTypeEdge, "size");
const GraphMLAttr* const headers_attr =
...
const vector<const GraphMLAttr* const> _all_graphml_attrs = {
    ...
    node_text_attr,
    node_type_attr,
    page_graph_edge_id_attr,
    page_graph_node_id_attr,
    page_graph_edge_time_attr,
    page_graph_node_time_attr,
    parent_node_attr,
    primary_pattern_attr,
    request_id_attr,
    request_type_attr,
    resource_type_attr,
    response_hash_attr,
    rule_attr,
    script_id_for_edge_attr,
    script_id_for_node_attr,
    script_position_attr,
    script_type_attr,
    secondary_pattern_attr,
    source_attr,
    status_type_attr,
    success_attr,
    tag_attr,
...
void GraphMLAttr::AddValueNode(xmlDocPtr doc,
  ...
  xmlNodePtr new_node =
      xmlNewChild(parent_node, NULL, BAD_CAST "data", encoded_content);
...
void GraphMLAttr::AddValueNode(xmlDocPtr doc,
  ...
  PG_LOG_ASSERT(type_ == kGraphMLAttrTypeLong);
  xmlNodePtr new_node =
      xmlNewTextChild(parent_node, NULL, BAD_CAST "data",
