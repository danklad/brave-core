...
void AdsServiceImpl::InitializePrefChangeRegistrar() {
  ...
  pref_change_registrar_.Add(ads::prefs::kEnabled,
                             base::BindRepeating(&AdsServiceImpl::OnPrefChanged,
                                                 base::Unretained(this)));

  pref_change_registrar_.Add(ads::prefs::kIdleTimeThreshold,
                             base::BindRepeating(&AdsServiceImpl::OnPrefChanged,
                                                 base::Unretained(this)));

  pref_change_registrar_.Add(brave_rewards::prefs::kWalletBrave,
                             base::BindRepeating(&AdsServiceImpl::OnPrefChanged,
                                                 base::Unretained(this)));

  pref_change_registrar_.Add(brave_news::prefs::kBraveTodayOptedIn,
                             base::BindRepeating(&AdsServiceImpl::OnPrefChanged,
                                                 base::Unretained(this)));

  pref_change_registrar_.Add(brave_news::prefs::kNewTabPageShowToday,
                             base::BindRepeating(&AdsServiceImpl::OnPrefChanged,
                                                 base::Unretained(this)));
...
void AdsServiceImpl::StartBatAdsService() {
  ...
  if (!IsBatAdsServiceBound()) {
    ...
    bat_ads_service_.set_disconnect_handler(base::BindOnce(
        &AdsServiceImpl::MaybeStart, AsWeakPtr(), /* should_restart */ true));
  }
...
void AdsServiceImpl::OnShutdownAndResetAllState(const bool success) {
  DCHECK(is_bat_ads_initialized_);
...
void AdsServiceImpl::OnPrefChanged(const std::string& path) {
  if (path == ads::prefs::kEnabled) {
    ...
    GetRewardsWallet();
  } else if (path == brave_news::prefs::kBraveTodayOptedIn ||
...
void AdsServiceImpl::CheckIdleState() {
  ...
  ProcessIdleState(idle_state, last_idle_at_);

  last_idle_at_ = ui::CalculateIdleTime();
}
...
int AdsServiceImpl::GetIdleTimeThreshold() const {
...
bool AdsServiceImpl::NeedsBrowserUpdateToViewAds() const {
...
void AdsServiceImpl::OnNotificationAdShown(const std::string& notification_id) {
  if (!IsBatAdsBound()) {
    return;
  }
...
void AdsServiceImpl::OnNotificationAdClosed(const std::string& notification_id,
                                            const bool by_user) {
  StopNotificationAdTimeOutTimer(notification_id);
...
void AdsServiceImpl::OnNotificationAdClicked(
...
void AdsServiceImpl::ResetAllState(const bool should_shutdown) {
  ...
  bat_ads_->Shutdown(
      base::BindOnce(&AdsServiceImpl::OnShutdownAndResetAllState, AsWeakPtr()));
}
...
void AdsServiceImpl::ShowNotification(const ads::NotificationAdInfo& ad) {
  if (ShouldShowCustomNotificationAds()) {
...
void AdsServiceImpl::OnURLRequest(
  ...
  if (response_code == kHttpUpgradeRequiredStatusCode &&
    ...
    for (AdsServiceObserver& observer : observers_) {
      observer.OnNeedsBrowserUpdateToViewAds();
    }
  }
