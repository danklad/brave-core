...
bool AdsClientMojoBridge::ShouldShowNotifications(bool* out_value) {
  DCHECK(out_value);
  *out_value = ads_client_->ShouldShowNotifications();
  return true;
}
...
bool AdsClientMojoBridge::CanShowBackgroundNotifications(bool* out_value) {
  DCHECK(out_value);
  *out_value = ads_client_->CanShowBackgroundNotifications();
  return true;
}
...
void AdsClientMojoBridge::ShowNotification(const std::string& json) {
...
void AdsClientMojoBridge::CloseNotification(const std::string& uuid) {
...
bool AdsClientMojoBridge::GetAdEvents(const std::string& ad_type,
  ...
  DCHECK(out_value);
  *out_value = ads_client_->GetAdEvents(ad_type, confirmation_type);
  return true;
}
