...
void GreaselionDownloadService::OnDATFileDataReady(std::string contents) {
  ...
  for (base::Value& rule_it : root_list) {
    ...
    base::Value::Dict& rule_dict = rule_it.GetDict();
    base::Value::Dict* preconditions_value = rule_dict.FindDict(kPreconditions);
    DCHECK(preconditions_value);
    base::Value::List* urls_value = rule_dict.FindList(kURLs);
    base::Value::List* scripts_value = rule_dict.FindList(kScripts);
