...
void Isolate::ReportBuiltInCallAndResponse(const char* func_name,
  ...
  if (V8_UNLIKELY(page_graph_backend_)) {
    page_graph_backend_->builtin_func_call_cb(
        reinterpret_cast<v8::Isolate*>(this), func_name, args);
    page_graph_backend_->builtin_func_response_cb(
        reinterpret_cast<v8::Isolate*>(this), func_name, result);
  }
}
