...
std::unique_ptr<base::DictionaryValue> ConstructAboutInformation(
  ...
  // Debug build on Ubuntu 22 all works even when keychain is locked, so keep
  ...
  Stat<bool>* is_os_keychain_unlocked =
      section_brave_sync.AddBoolStat("OS keychain unlocked");
  is_os_keychain_unlocked->Set(brave_sync::Prefs::IsOsKeychainUnlocked());
